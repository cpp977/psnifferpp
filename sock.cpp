...
int main(int argc, char* argv[])
{
    ...
    auto ether_type_filter_vec = args.get_multiple<std::string>("filter", "ETH_P_ALL");
    std::set<unsigned short> ether_type_filter;
    for(const auto& f : ether_type_filter_vec) { ether_type_filter.insert(eth_types.at(f)); }
    ...
    if(auto handle = sockpp::socket::create_handle(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL)); handle) {
        ...
        sockpp::sock_address_any interface_addr(reinterpret_cast<const sockaddr*>(&iface), sizeof(sockaddr_ll));
        ...
        while(true) {
            ...
            if(auto recv = sock.recv(msg.data(), msg.size()); recv) {
                ...
                if(*tpid == 0x8100) {
                    ...
                    std::format_to(std::back_inserter(eth_hdr_string), "Received data through raw socket (size={}).\n", recv.value());
                    std::format_to(std::back_inserter(eth_hdr_string), "  Ethernet header:\n");
                    ...
                    std::format_to(std::back_inserter(eth_hdr_string),
                                   ...
                                   eth->h_dest[4],
                                   eth->h_dest[5]);
                    std::format_to(std::back_inserter(eth_hdr_string), "    type={0} ({0:04X})\n", ntohs(ether_type));
                    if(interface == "tun0") { ether_type = htons(ETH_P_IPV6); }
                    switch(ntohs(ether_type)) {
                    case ETH_P_IP: {
                        if(not ether_type_filter.contains(ETH_P_IP) and not ether_type_filter.contains(ETH_P_ALL)) { break; }
                        if(interface != "tun0") { std::cout << eth_hdr_string; }
                        const iphdr* ip = reinterpret_cast<const iphdr*>(msg.data() + ETH_HDR_SIZE);
                        ...
                        std::format_to(std::back_inserter(ip_hdr_string), "  IPv4 header:\n");
                        std::format_to(std::back_inserter(ip_hdr_string), "    version={:d}\n", ip->version);
                        std::format_to(std::back_inserter(ip_hdr_string), "    type of service={}\n", static_cast<unsigned int>(ip->tos));
                        std::format_to(std::back_inserter(ip_hdr_string), "    protocol={}\n", ip->protocol);
                    ...
                    case ETH_P_IPV6: {
                        if(not ether_type_filter.contains(ETH_P_IPV6) and not ether_type_filter.contains(ETH_P_ALL)) { break; }
                        if(interface != "tun0") { std::cout << eth_hdr_string; }
                        const ipv6hdr* ip = reinterpret_cast<const ipv6hdr*>(msg.data() + ETH_HDR_SIZE);
                        ...
                        std::format_to(std::back_inserter(ip6_hdr_string), "  IPv6 header:\n");
                        std::format_to(std::back_inserter(ip6_hdr_string), "    version={:d}\n", ip->version);
                        std::format_to(std::back_inserter(ip6_hdr_string), "    protocol={}\n", ip->nexthdr);
                    ...
                    case ETH_P_ARP: {
                        if(not ether_type_filter.contains(ETH_P_ARP) and not ether_type_filter.contains(ETH_P_ALL)) { break; }
                        if(interface != "tun0") { std::cout << eth_hdr_string; }
                        const arphdr* arp = reinterpret_cast<const arphdr*>(msg.data() + ETH_HDR_SIZE);
                        std::format_to(std::back_inserter(arp_hdr_string), "  ARP header:\n");
                        sockpp::inet_address src_ip(ntohl(*reinterpret_cast<const uint32_t*>(msg.data() + ETH_HDR_SIZE + sizeof(arphdr) + 6)), 0);
                        sockpp::inet_address dst_ip(ntohl(*reinterpret_cast<const uint32_t*>(msg.data() + ETH_HDR_SIZE + sizeof(arphdr) + 16)), 0);
                    ...
                    case ETH_P_IEEEPUP: {
                        if(not ether_type_filter.contains(ETH_P_IEEEPUP) and not ether_type_filter.contains(ETH_P_ALL)) { break; }
                        if(interface != "tun0") { std::cout << eth_hdr_string; }
                        std::format_to(std::back_inserter(pup_hdr_string), "  PUP header:\n");
                        const uint8_t* src_addr = reinterpret_cast<const uint8_t*>(msg.data() + ETH_HDR_SIZE + 14);
                        const uint8_t* src_host = reinterpret_cast<const uint8_t*>(msg.data() + ETH_HDR_SIZE + 15);
                        const uint32_t* src_socket = reinterpret_cast<const uint32_t*>(msg.data() + ETH_HDR_SIZE + 16);
                        const uint8_t* dst_addr = reinterpret_cast<const uint8_t*>(msg.data() + ETH_HDR_SIZE + 8);
                        const uint8_t* dst_host = reinterpret_cast<const uint8_t*>(msg.data() + ETH_HDR_SIZE + 9);
                        const uint32_t* dst_socket = reinterpret_cast<const uint32_t*>(msg.data() + ETH_HDR_SIZE + 10);
                        std::format_to(std::back_inserter(pup_hdr_string), "    src addr={}\n", *src_addr);
                        std::format_to(std::back_inserter(pup_hdr_string), "    src port={}\n", *src_host);
                        std::format_to(std::back_inserter(pup_hdr_string), "    src socket={}\n", *src_socket);
                        std::format_to(std::back_inserter(pup_hdr_string), "    dst addr={}\n", *dst_addr);
                        std::format_to(std::back_inserter(pup_hdr_string), "    dst port={}\n", *dst_host);
                        std::format_to(std::back_inserter(pup_hdr_string), "    dst socket={}\n", *dst_socket);
                        std::cout << pup_hdr_string;
                        break;
                    }
                    default: {
                        if(not ether_type_filter.contains(MY_ETH_OTHER) and not ether_type_filter.contains(ETH_P_ALL)) { break; }
                        if(interface != "tun0") { std::cout << eth_hdr_string; }
                        std::format_to(std::back_inserter(other_hdr_string), "  Other header:\n");
...
void hexdump(std::span<char> buffer)
{
    ...
    for(auto count = 0; auto c : buffer) {
        if(count % 6 == 0 and count > 0) { std::cout << std::endl; }
